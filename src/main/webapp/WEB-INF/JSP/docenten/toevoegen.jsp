...
<body>
	...
	<form method='post' id='toevoegform'>
		...
		<label>Rijksregisternummer:
			<span>${fouten.rijksregisternr}</span> 
			<input name='rijksregisternr' value='${param.rijksregisternr}' 
			required type='number' min='10000000000' max='99999999999'>
		</label> 
		
		<label>Campus: 
			<span>${fouten.campussen}</span> 
			<select
				name='campussen' size='${campussen.size()}' required>
					<c:forEach items='${campussen}' var='campus'>
						<option value='${campus.id}' ${campus.id == param.campussen ? 'selected' : ''}>
							${campus.naam} (${campus.adres.gemeente})
						</option>
					</c:forEach>
			</select>
		</label>
